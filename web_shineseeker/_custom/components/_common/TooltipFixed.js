...
// Tooltip 컴포넌트
const Tooltip = ({children, content, css = "", tooltipCss = "", style = {}, reverse = false, maxWidth = 450, fixWidth = null, baseLeft = 50, fixLeft = null}) => {
  ...
  useEffect(() => {
    ...
    if (content?.props?.children?.length && tooltipContainerRef.current) {
      ...
      if (tooltipElement) {
        ...
        tempLeft = left + Math.floor((containerWidth - (tooltipWidth < 150 ? 150 : tooltipWidth)) / 2 + 5); // 부모의 가로 중앙에 툴팁 위치
