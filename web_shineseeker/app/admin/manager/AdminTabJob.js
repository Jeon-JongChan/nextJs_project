...
export default function Home() {
  ...
  async function fetchEssentialData() {
    ...
    devLog("essential data skill detail: ", newData);
  }
            ...
            if (maindata[key]["job_name"]) {
              return (
                <Tooltip key={index} content={null} css={"w-full"}>
                  <ListItemIndex label={maindata[key]["job_name"]} index={index} onclick={clickListItem} deleteButton={true} deleteFunc={deleteTarget} alignDir={"left"} />
                </Tooltip>
              );
            }
          })}
