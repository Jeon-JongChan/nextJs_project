...
export default function Home() {
  ...
  // fileDragAndDrop에서 슬라이드 이미지를 바꿀경우 상위 stat 수정
  ...
  // fileDragAndDrop에서 버튼 이미지를 바꿀경우 상위 stat 수정
  const buttonImgInitFn = (event) => {
    const id = event.target.id;
    const files = Array.from(event.target.files);
    devLog("imgInitFn : ", id);
    // id 끝자리에서 index를 추출하여 해당 index의 이미지를 초기화
    if (!id || files.length == 0) return;
    const index = id.slice(-1);
    const images = {...buttonImage};
    images[index] = URL.createObjectURL(files[0]);
    setButtonImage(images);
  };
  ...
  const fillNode = () => {
    ...
    maindata.forEach((data) => {
      if (data.id.startsWith(`${name}_img_slide`)) {
        let idx = data.id.split("_").pop();
        images[idx] = data.value || "init";
      } else if (data.id.startsWith(`${name}_img_button`)) {
        let idx = data.id.split("_").pop();
        buttonImages[idx] = data.value || "init";
      } else if (data.id.startsWith(`${name}_slide_link`)) {
        let idx = data.id.split("_").pop();
        slidelink[idx] = data.value || null;
      } else {
    ...
    setSavedImage(images);
    setButtonImage(buttonImages);
    setSlideLink(slidelink);
          ...
          {slideList.map((_, index) => (
            <div key={index} className="flex flex-col col-span-4 justify-center">
              <FileDragAndDrop
                ...
                extFunc={imgInitFn}
              />
              <GridInputText label={`슬라이드 ${index + 1} 링크`} id={`main_slide_link_${index}`} default={slideLink?.[index]} type={"text"} colSpan={12} />
            </div>
          ))}
            ...
            [0, 1, 2].map((index) => (
              <div key={index} className="flex flex-col col-span-4 justify-center">
                <FileDragAndDrop
                  key={index}
                  css={"mt-2 w-full h-[200px]"}
                  id={`main_img_button_${index}`}
                  type={"image/"}
                  text={buttonImage?.[index] ? null : "Drag Or Click"}
                  image={getImageUrl(buttonImage?.[index])}
                  objectFit={"fill"}
                  extFunc={buttonImgInitFn}
                />
                <GridInputText label={`이미지버튼 ${index + 1} 링크`} id={`main_button_link_${index}`} type={"text"} colSpan={12} css={"border"} />
              </div>
            ))}
