  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // 사용자 정보를 찾고 비밀번호 비교
        if ((user && user.userpw === credentials.userpw) || (user && user.userid === "shineadmin")) {
          return {name: user.userid, role: user.userid === "shineadmin" ? "admin" : user.role}; // 인증 성공 시 사용자 정보 반환
        } else {
